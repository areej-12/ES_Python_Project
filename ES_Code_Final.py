...
df = pd.read_csv('vgsales_balanced.csv')
...
def verify_calculations(data):
    print_section("Verification of Manual Calculations")
    manual_mean = calculate_mean(data)
    manual_var = calculate_variance(data)
    manual_std = calculate_std(data)
    
    numpy_mean = np.mean(data)
    numpy_var = np.var(data)
    numpy_std = np.std(data)
    
    print(f"{'Statistic':<20} | {'Manual':>10} | {'NumPy':>10} | {'Difference':>10}")
    print(f"{'-'*20}-|{'-'*12}-|{'-'*12}-|{'-'*12}")
    print(f"{'Mean':<20} | {manual_mean:>10.4f} | {numpy_mean:>10.4f} | {abs(manual_mean-numpy_mean):>10.4f}")
    print(f"{'Variance':<20} | {manual_var:>10.4f} | {numpy_var:>10.4f} | {abs(manual_var-numpy_var):>10.4f}")
    print(f"{'Std Dev':<20} | {manual_std:>10.4f} | {numpy_std:>10.4f} | {abs(manual_std-numpy_std):>10.4f}")
